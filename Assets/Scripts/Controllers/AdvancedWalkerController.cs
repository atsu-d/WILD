...
namespace PlayerController
{
	...
	public class AdvancedWalkerController : Controller {
		...
		public float sprintSpeed = 6f; //potential added speed, not total
		public float baseMovementSpeed = 7f;
		private float sprintTime = 0f;
		public float timeMult = 0.3f;
		...
		void Awake () {
			...
			if (PlayerTR == null) PlayerTR = this.transform;
		}
		...
		float HandleSpeed()
        {
			...
			_movementSpeed = baseMovementSpeed + (_sprintKeyDown ? accelCurve.Evaluate(sprintTime): decelCurve.Evaluate(sprintTime)) * sprintSpeed;
